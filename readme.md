...
* `/start`, `/help` — справка
* `/chat_id` — показать ID чата (нужен, чтобы привязать чат в личке)
* `/summ DD.MM.YY` — summary за указанный день (пример: `/summ 18.01.26`)
...
* `/import` - *только для админа чата* импортирует историю чата в формате json.

**В личке**

* `/start`, `/help` — справка
* отправить `chat_id` — привязать чат
* `/summ` — выбрать чат и период, за который делать суммаризацию
